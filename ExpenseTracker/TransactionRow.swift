...
struct TransactionRow: View {
        ...
        HStack(spacing: 20) {
            ...
            if let categoryName = transaction.category?.name {
                RoundedRectangle(cornerRadius: 20, style: .continuous)
                    ...
                    .overlay {
                        FontIcon.text(.awesome5Solid(code: iconForCategory(categoryName: categoryName)), fontsize: 24, color: MyColors.icon)
                    }
            }
    ...
    func iconForCategory(categoryName: String) -> FontAwesomeCode {
        switch categoryName {
        case "Groceries":
            return .shopping_bag // lub .shoppingBag jeśli .shoppingBasket nie istnieje
        case "Transport":
            return .bus
        case "Clothes":
            return .tshirt
        case "Subscriptions":
            return .credit_card // lub .creditCard jeśli .moneyCheckAlt nie istnieje
        default:
